...
test("loadNativeLibraryPath resolves an explicit library path argument first", async () => {
  ...
  expect(loadNativeLibraryPath(explicitRelativePath)).toBe(resolve(explicitRelativePath));
});
...
test("loadNativeLibraryPath throws with looked-up candidates when not found", async () => {
  ...
  const { loadNativeLibraryPath } = await importPathResolver(`?missing=${Date.now()}`);

  expect(() => loadNativeLibraryPath()).toThrow("Could not find the Sudachi native library.");
  ...
  try {
    ...
    expect(message).toContain(join(resolve(nestedExplicitDir), `libsudachi_ffi.${suffix}`));
    expect(message).toContain(join(resolve(nestedExplicitDir), `sudachi_ffi.${suffix}`));
  }
});
