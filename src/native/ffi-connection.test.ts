...
function resolveNativeLibraryPath(): string | null {
  ...
  const candidates = [
    resolve(PROJECT_ROOT, "sudachi-ffi", "target", "release", `libsudachi_ffi.${suffix}`),
    resolve(PROJECT_ROOT, "sudachi-ffi", "target", "release", `sudachi_ffi.${suffix}`),
    resolve(PROJECT_ROOT, "sudachi-ffi", "target", "debug", `libsudachi_ffi.${suffix}`),
    resolve(PROJECT_ROOT, "sudachi-ffi", "target", "debug", `sudachi_ffi.${suffix}`),
  ];
  return candidates.find((path) => fileExists(path)) ?? null;
}

function resolveDictionaryPath(): string | null {
  const explicit = process.env.SUDACHI_TEST_DICT_PATH?.trim()
  ...
  try {
    ...
    const nestedCandidates = Array.from(
      new Bun.Glob("sudachi-dictionary-*/system_core.dic").scanSync({ cwd: dictRoot, absolute: true }),
    );
...
const nativeTest = nativeLibraryPath === null || !canLoadCurrentNativeAbi ? test.skip : test;
const dictTest = nativeLibraryPath === null || !canLoadCurrentNativeAbi || dictPath === null ? test.skip : test;

nativeTest("FFI: can load all native libraries and call common error/status symbols", () => {
  const options = { libraryPath: nativeLibraryPath! };
  ...
  try {
    for (const library of [tokenizerLibrary, lookupLibrary, pretokenizerLibrary, sentenceSplitterLibrary]) {
      expect(typeof String(library.symbols.sudachi_get_last_error())).toBe("string");
      expect(typeof String(library.symbols.sudachi_status_code_name(0))).toBe("string");
    }
  } finally {
...
nativeTest("FFI: can read all result layouts from the real native library", () => {
  const options = { libraryPath: nativeLibraryPath! };
  const tokenizerLibrary = loadNativeLibrary(options);
  const lookupLibrary = loadLookupLibrary(options);
  const pretokenizerLibrary = loadPretokenizerLibrary(options);
  ...
  try {
    ...
    const pretokenizedLayout = readPretokenizedResultLayout(pretokenizerLibrary);
    ...
    const sentenceSpanLayout = readSentenceSpanResultLayout(sentenceSplitterLibrary);
...
nativeTest("FFI: constructor APIs report nativeStatus when dictionary path is invalid", () => {
  const missingDictPath = resolve(PROJECT_ROOT, ".tmp", "missing-system.dic");

  expect(() => createPretokenizer({ dictPath: missingDictPath, libraryPath: nativeLibraryPath! })).toThrow(
    SudachiError,
  );
  expect(() => createSentenceSplitter({ dictPath: missingDictPath, libraryPath: nativeLibraryPath! })).toThrow(
    SudachiError,
  );

  try {
    createPretokenizer({ dictPath: missingDictPath, libraryPath: nativeLibraryPath! });
  ...
  try {
    createSentenceSplitter({ dictPath: missingDictPath, libraryPath: nativeLibraryPath! });
...
dictTest("FFI: createSudachi smoke test succeeds with a real dictionary", () => {
  ...
  try {
    ...
    const sentenceSpans = sudachi.splitter.split("今日は晴れです。明日も晴れです。");
